...
void separaAlfanumVarias(char sequencia[], char resultado[])
{
    int z = 0;
    char x = '\0';
    for( int i = 0; sequencia[i] != '\0'; i++ )
...
void method_10 (void)
{
    ...
    int umaVez = 0;
    char sequencia[100];
    char resultado[100]; //OBS: (resultado * n) tem caber em final, CUIDADO!
    char final[100];
    printf("Method_10 - Ex0420");
    
    while(n<=0)
    {
        printf("\nDigite o numero de sequencias de caracteres: ");
        scanf( "%d", &n );   getchar( );
    }
    ...
    while(n>0)
    {
        ...
        separaAlfanumVarias(sequencia, resultado); 
        if(umaVez == 0)     // so faz isso na primeira vez
        {
            strcpy(final, resultado);
            umaVez = umaVez + 1;
        }
        else                // se nao fora a primeira vez vai juntando no final
        {
            strcat(final, resultado);
        }
        n = n - 1;          // decrementa o numero de sequencias de caracteres (n)
    }
    printf("\n%s","Caracteres ALFANUMERICOS: { ");
        
    // enquanto (i) for menor que o tamanho total de resultado (sizeof) e nao for igual ao '\0'
    // exibe os valores de resultado
    for(int i = 0; (double)i < sizeof(final) && final[i] != '\0'; i++) //OBS: sizeof e prevencao
    {
        if(i != 0)  // garante que o primeiro caractere nao tenha virgula antes
        {
            // poe a virgula antes do proximo caractere
        ...
        printf("%c", final[i]); // printa o caractere
    }
    printf("%s"," }");
...
int main ( )
{
    ...
    {
        // identificar
        printf( "EXERCICIO420 - Programa - v0.0" );
        printf("\nAutor: Daniel Alves Goncalves");
        // ler do teclado
        printf ( "\n%s\n","Opcoes" );
        printf ( "\n%s","0 - Parar");
...
Esse deu trabalho tambem, mas consegui juntar tudo em uma string contendo a resposta.
...
---------------------------------------------- previsao de testes
{ (A1b2+C3d4)*E5f6, [P&&Q]||[R&&!S], (a<b&&b<c) }
---------------------------------------------- historico
